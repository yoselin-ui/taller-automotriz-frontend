...
export default function Empleados() {
  ...
  useEffect(() => {
    ...
    if (searchTerm === '') {
      ...
      const filtered = empleados.filter(empleado =>
        ...
        (empleado.especialidad && empleado.especialidad.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  if (loading) return <div style={{ padding: '40px', textAlign: 'center' }}>Cargando empleados...</div>;
  ...
  return (
    <div className="empleados-container">
      ...
      <div className="employees-grid">
        {empleadosFiltrados.length > 0 ? (
          empleadosFiltrados.map((empleado) => (
            <div key={empleado.id_empleado} className={`employee-card ${!empleado.activo ? 'inactive' : ''}`}>
              ...
              <div className="employee-actions">
                <button className="btn-icon" onClick={() => handleEdit(empleado)}>✏️</button>
                <button className="btn-icon" onClick={() => handleToggle(empleado.id_empleado)}>
                  {empleado.activo ? '🔒' : '✅'}
                </button>
                <button className="btn-icon" onClick={() => handleDelete(empleado.id_empleado)}>🗑️</button>
              </div>
            </div>
          ))
        ) : (
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <input type="text" value={formData.especialidad} onChange={(e) => setFormData({ ...formData, especialidad: e.target.value })} placeholder="Ej: Mecánico General" />
              </div>
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="btn-primary">{editingEmpleado ? 'Actualizar' : 'Crear'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <style jsx>{`
        .empleados-container { animation: fadeIn 0.5s ease; }
        ...
        .employees-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 24px; }
        .employee-card { background: #1a1a1a; border: 1px solid #333; border-radius: 16px; padding: 32px; text-align: center; transition: all 0.3s; }
        .employee-card.inactive { opacity: 0.6; border-color: #555; }
        .employee-card:hover { transform: translateY(-4px); border-color: #E50914; box-shadow: 0 8px 32px rgba(229, 9, 20, 0.2); }
        .employee-avatar { width: 80px; height: 80px; background: linear-gradient(135deg, #E50914 0%, #b20710 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 40px; margin: 0 auto 20px; }
        .employee-card h3 { font-size: 20px; font-weight: 700; margin-bottom: 12px; }
        .employee-specialty { background: rgba(229, 9, 20, 0.2); color: #E50914; padding: 6px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; display: inline-block; margin-bottom: 12px; }
        .employee-contact { color: #999; font-size: 14px; margin-bottom: 8px; }
        .employee-salary { color: #4CAF50; font-size: 16px; font-weight: 700; margin-bottom: 12px; }
        .employee-status { margin-bottom: 16px; }
        ...
        .employee-actions { display: flex; justify-content: center; gap: 12px; }
        .btn-icon { background: rgba(229, 9, 20, 0.1); border: 1px solid rgba(229, 9, 20, 0.3); border-radius: 8px; padding: 10px 16px; cursor: pointer; font-size: 18px; transition: all 0.2s; }
